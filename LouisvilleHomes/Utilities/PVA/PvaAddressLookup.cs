...
namespace LouisvilleHomes.Utilities.PVA
{
    public class PvaAddressLookup : IPvaAddressLookup
    {
        ...
        public PvaAddressLookup(PvaSettings settings, IAddressParser addressParser, ILogger<PvaAddressLookup> logger)
        {
            _settings = settings;
        ...
        public async Task<PvaPropertyInfo> GetPropertyInfo(string address)
        {
            ...
            var baseAddressString = string.Concat(bestParsing.Number, " ", (string.IsNullOrWhiteSpace(bestParsing.Direction) ? "" : bestParsing.Direction + " "), bestParsing.Street, (string.IsNullOrWhiteSpace(bestParsing.Tag) ? "" : " " + bestParsing.Tag));

            try
            {
                var document1 = webGet.Load(_settings.Url + string.Format(_settings.SearchUrl, HttpUtility.UrlEncode(baseAddressString)));
                ...
                else if (!redirectScriptTag.Any() && !string.IsNullOrEmpty(bestParsing.Tag))
                {
                ...
                document2 = document2 ?? webGet.Load(string.Concat(_settings.Url, redirectUrl));
                ...
                {
                    _logger.LogError("PVA returned differenct address than searched: (original) {address} (returned) {returnedAddress}", address, returnedAddress);
                ...
                propertyInfo.Owners = HttpUtility.HtmlDecode(ownerNames);
                ...
                else
                {
                    ...
                    {
                        propertyInfo.AssessedValue = iAssessedValue;
                    }                    
                }
